...
{
    public class StartDeliveryCommandHandler : IRequestHandler<StartDeliveryCommand, OrderResponseDto>
    {
        private readonly ApplicationDBContext _context;

        public StartDeliveryCommandHandler(ApplicationDBContext context)
        {
            _context = context;
        }

        public async Task<OrderResponseDto> Handle(StartDeliveryCommand request, CancellationToken cancellationToken)
        {
            ...
            if (order.Status != "Accepted")
                throw new InvalidOperationException("Order must be accepted before starting delivery");

            if (order.Trip?.TravelerId != request.UserId)
                throw new UnauthorizedAccessException("Only the traveler can start delivery");
